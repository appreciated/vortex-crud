            ...
            if (kanbanRoute.rowIndexField() != null) {
                ...
                if (dataStore instanceof VortexCrudQueryDataStore) {
                    targetColumnItems = ((VortexCrudQueryDataStore<FieldType, Object>)dataStore).getRecordsFromTableWhereColumnEqualsOrdered(
                     ...
                     // Fallback: fetch all, filter by column, sort by index
                     targetColumnItems = dataStore.getRecordsFromTable(0, 1000).stream()
                        .filter(item -> Objects.equals(reflectionService.getValue(item, kanbanRoute.columnField()), columnDatabaseValue))
                        .sorted(Comparator.comparing(o -> (Comparable) reflectionService.getValue(o, kanbanRoute.rowIndexField())))
                        .toList();
                }
