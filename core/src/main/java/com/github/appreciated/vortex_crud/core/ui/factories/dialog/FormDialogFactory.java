...
public class FormDialogFactory<ModelClass, FieldType, RepositoryType> implements VortexCrudDialogFactory<ModelClass, FieldType, RepositoryType> {
    ...
    public Dialog create(@Nullable Object entityId,
        ...
        if (formRouteRenderer instanceof MultiFormRoute) {
            ...
            for (FormRoute<ModelClass, FieldType, RepositoryType> childForm : multiFormRoute.forms()) {
                ...
                formCreator.bindAndAddToLayout(dataStoreKey, formRouteRenderer, childForm.fields(), recordById, context, tables, binder, childFormLayout);
                formsContainer.add(childFormLayout);
            }
            formContent = formsContainer;
        } else if (formRouteRenderer instanceof FormRouteProvider<ModelClass,FieldType,RepositoryType>){
            // Single form configuration
            FormLayout layout = new FormLayout();
            formCreator.bindAndAddToLayout(dataStoreKey, formRouteRenderer, ((FormRouteProvider<ModelClass, FieldType, RepositoryType>) formRouteRenderer).fields(), recordById, context, tables, binder, layout);
            formContent = layout;
        }
