...
public class DefaultCardItem<FieldType> extends Card {
    ...
    public DefaultCardItem(ItemFactory<FieldType> config,
        ...
        if (imageFieldType != null) {
            ...
            String imagePath = context.reflectionService().getString(entity, imageFieldType);
            image = new ImageDisplayComponent(context.fileProviderRegistry().getFactory(config.resourceProvider()));
            image.setImageSource(imagePath);
        }

        H4 title = new H4(context.reflectionService().getString(entity, config.titleField()));
        ...
        if (config.descriptionField() != null) {
            Text description = new Text(context.reflectionService().getString(entity, config.descriptionField()));
