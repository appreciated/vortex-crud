...
public class ListCollectionFactory<ModelClass, FieldType, RepositoryType> implements VortexCrudCollectionFactory<ModelClass, FieldType, RepositoryType> {
    ...
    public Component createCollection(Object foreignKey,
        ...
        button.addClickListener(event -> openDialog(null, foreignKey, factoryConfig, context, list, header));
        header.add(button);
        loadCollection(foreignKey, factoryConfig, context, list, header);
        return list;
    }
    ...
    private void loadCollection(Object foreignKeyValue,
        ...
        VortexCrudDataStore<FieldType, ModelClass> dataStore = context.configService().configuration().dataStores().get(data.dataStore()).dataStoreInstance();

        java.util.Collection<Object> records = (data.manyToMany() != null) ?
                (java.util.Collection<Object>) context.manyToManyPersistenceStrategy().resolveManyToMany(dataStore, data.manyToMany(), foreignKeyValue) :
        ...
        if (internalFormElement.configuration().data().oneToMany() != null) {
            addOneToManyItems(foreignKeyValue, internalFormElement, context, list, header, records, dataStore);
        } else if (internalFormElement.configuration().data().manyToMany() != null) {
            addManyToManyItems(foreignKeyValue, internalFormElement, context, list, header, records, dataStore);
        } else {
    ...
    private void addManyToManyItems(Object foreignKeyValue,
        ...
        for (Object record : records) {
            ...
            item.getContent().addClickListener(event -> openDialog(foreignKeyValue, foreignKeyValue, internalFormElement, context, list, header));
            List<FieldType> children = internalFormElement.configuration().data().children();
            children.forEach(fieldId -> item.addContent(new Text(context.reflectionService().getString(record, fieldId))));
            Button remove = new Button(VaadinIcon.TRASH.create());
            remove.addThemeVariants(LUMO_TERTIARY_INLINE, LUMO_SMALL, LUMO_ERROR);
            remove.addClickListener(event -> {
                dataStore.deleteRecordById(context.dataStoreUtil().getId(record));
                loadCollection(foreignKeyValue, internalFormElement, context, list, header);
            });
    ...
    private void addOneToManyItems(Object foreignKeyValue,
        ...
        for (Object record : records) {
            DefaultCollectionItem item = new DefaultCollectionItem();
            item.getContent().addClickListener(event -> openDialog(context.reflectionService().getId(record), foreignKeyValue, internalFormElement, context, list, header));
            ...
            for (InternalFormElement<ModelClass, FieldType, RepositoryType> child : form.children()) {
                String textValue = context.reflectionService().getString(record, child.field());
