...
public class ListCollectionFactory<ModelClass, FieldType, RepositoryType> implements VortexCrudCollectionFactory<ModelClass, FieldType, RepositoryType> {
    ...
    private void loadCollection(Object foreignKeyValue,
                                Collection<ModelClass, FieldType, RepositoryType> data,
                                VerticalLayout list,
        ...
        if (data.oneToMany() != null) {
            addOneToManyItems(foreignKeyValue, data, list, header, records, dataStore, context);
        } else if (data.manyToMany() != null) {
            addManyToManyItems(foreignKeyValue, data, list, header, records, dataStore, context);
        } else {
            throw new IllegalArgumentException("No collection found for " + foreignKeyValue);
        }
        if (records.isEmpty()) {
            list.add(new Text(list.getTranslation(data.emptyMessage())));
        }
    }

    private void addManyToManyItems(Object foreignKeyValue,
                                    Collection<ModelClass, FieldType, RepositoryType> internalFormElement,
                                    VerticalLayout list,
                                    HorizontalLayout header,
    ...
    private void openDialog(Object entityId,
                            ...
                            HorizontalLayout header,
                            VortexCrudContext<ModelClass, FieldType, RepositoryType> context) {
        Collection<ModelClass, FieldType, RepositoryType> collectionData = internalFormElement;
        ...
        com.vaadin.flow.component.dialog.Dialog dialog = collectionData.dialogFactory().create(
                entityId,
                foreignKeyValue,
                referenceField,
                collectionData.form(),
