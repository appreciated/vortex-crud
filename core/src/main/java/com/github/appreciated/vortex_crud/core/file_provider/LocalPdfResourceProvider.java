...
/**
 * Resource provider for PDF files.
 * PDFs are stored in the configured directory.
 */
public class LocalPdfResourceProvider implements VortexCrudResourceProvider {

    private static final Logger log = LoggerFactory.getLogger(LocalPdfResourceProvider.class);
    ...
    private void ensureStorageDirectoryExists() {
        try {
            ...
            if (!Files.exists(path)) {
                ...
                log.info("Created PDF storage directory: {}", path.toAbsolutePath());
            }
        } catch (IOException e) {
            log.error("Failed to create PDF storage directory: {}", basePath, e);
        }
    }
