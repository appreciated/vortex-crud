...
/**
 * Resource provider for generic files.
 * Files are stored in the configured directory.
 */
public class LocalFileResourceProvider implements VortexCrudResourceProvider {

    private static final Logger log = LoggerFactory.getLogger(LocalFileResourceProvider.class);
    ...
    private void ensureStorageDirectoryExists() {
        try {
            ...
            if (!Files.exists(path)) {
                ...
                log.info("Created file storage directory: {}", path.toAbsolutePath());
            }
        } catch (IOException e) {
            log.error("Failed to create file storage directory: {}", basePath, e);
        }
    }
