...
public class FormRouteFactory<ModelClass, FieldType, RepositoryType> implements VortexCrudRouteFactory<ModelClass, FieldType, RepositoryType> {
    ...
    public Component renderRoute(
        ...
        return getForm(routeResolver, detailRouteSetting.isWrapped(), detailRouteSetting.isHeaderHidden(), detailRouteSetting.isCreationMode(), routeProvider.isDeleteButtonHidden(), routeProvider, form, context);
    }
    ...
    public VerticalLayout getForm(VortexCrudPathToRouteResolver<ModelClass, FieldType, RepositoryType> routeResolver,
        ...
        if (!creationMode) {
            binder.bindReadOnly(
                    titleComponent,
                    entity1 -> prefix + context.reflectionService().getString(entity1, formRouteRendererConfiguration.titleField())
            );
        } else {
        ...
        context.formCreator().bindAndAddToLayout(table, routeRenderer, formRouteRendererConfiguration.children(), entity, tables, binder, form, context);
