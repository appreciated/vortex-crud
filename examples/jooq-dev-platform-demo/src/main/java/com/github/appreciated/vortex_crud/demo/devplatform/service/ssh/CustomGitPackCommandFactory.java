...
public class CustomGitPackCommandFactory extends GitPackCommandFactory {
    ...
    public Command createCommand(ChannelSession channel, String command) throws IOException {
        ...
        if (args.length >= 2) {
            ...
            Integer userId = channel.getSession().getAttribute(DatabasePublickeyAuthenticator.USER_ID_KEY);
            ...
            if ("git-upload-pack".equals(action)) {
                ...
                // Unknown command or other git command?
                // git-upload-archive?
                allowed = false;
            }

            if (!allowed) {
                // Return a command that fails immediately
                return new Command() {
                    @Override
                    ...
                    public void start(ChannelSession channel, org.apache.sshd.server.Environment env) throws IOException {
                        // Should be handled by exit callback, but we can also throw here
                    }
                    @Override
